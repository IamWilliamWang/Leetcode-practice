...
def eraseAt(s: str, eraser=1):
    eraseIdx = len(s)

    def isHuiwenErasable(i, j):
        nonlocal eraser, eraseIdx
        while i < j:
            if s[i] != s[j]:
                if eraser > 0:
                    eraser -= 1
                    if isHuiwenErasable(i, j - 1):
                        eraseIdx = j
                        return True
                    elif isHuiwenErasable(i + 1, j):
                        eraseIdx = i
                        return True
                    return False
                return False
    ...
    if isHuiwenErasable(0, len(s) - 1):
        return eraseIdx  # 返回擦除位置，如果不用擦除就返回len(s)
    return -1  # 不满足就返回-1


minLimit, maxLimit = list(map(int, input().strip().split()))
primes = Prime()
ans = 0
for i in range(minLimit, maxLimit + 1):
    num = str(i)
    eraser = 1
    while num:
        removedChIdx = eraseAt(num, eraser)
        if removedChIdx == -1:  # 无论如何都不能成为回文
            break
        pureNum = int(num[:removedChIdx] + num[removedChIdx + 1:])  # 制造回文数
        if pureNum in primes and i != pureNum:  # 满足回文素数，并一定去掉过某个数字
            # print(i, '->', pureNum)
            ans += 1
            break
        if not eraser:  # 不能擦除，放弃
            break
        if eraseAt(num[:-1], eraser - 1) != -1:
            num = num[:-1]
        elif eraseAt(num[1:], eraser - 1) != -1:
            num = num[1:]
        else:
            break
        eraser -= 1
