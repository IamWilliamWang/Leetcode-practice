...
# region 常用算法
class Primes:  # mao1112
    """
    获得素数数组的高级实现。支持 in 和 [] 运算符，其中[]支持取一个和取多个
    """
    ...
    def __contains__(self, n: int) -> bool:
        ...
        if not n % 2:  # 偶数只有2是素数
            return n == 2
        a, b = self.indexOf(n)  # 搜索n在素数列表中的位置(1-based index)
        return a == b  # 如果a b相同，代表这就是n在素数列表中的位置

    def __getitem__(self, i):
        """
        获得第i个元素，或者根据slice返回切片数组（只有当i.stop=None时返回迭代器）
        :param i: index或者slice
        :return:
        """
        if isinstance(i, slice):  # 如果i是切片
            if i.stop:  # 如果有指明stop的话
                self._extendToLen(i.stop + 1)  # 拓宽list到stop位置
                return self._list[i]  # 将切片对应的数组返回
            return islice(self, i.start, None)  # 只有不指定stop时制造迭代器的切片，丢弃n.start之前的元素，返回指向n.start位置的迭代器
        else:  # 如果n是正整数
    ...
    def _extend(self, n: int) -> None:
        ...
        for prime in self.primeRange2(2, maxSqrt):  # 得到[2,maxSqrt)所有的素数
            for i in range(-begin % prime, len(sizer), prime):
                sizer[i] = 0  # 把所有非素数变为0
        self._list.extend([x for x in sizer if x])  # 删去所有的零，剩下的就是素数列表

    def _extendToLen(self, listLen: int) -> None:
        """
        将素数列表的长度**至少**拓展到listLen
    ...
    def indexOf(self, n: int) -> Tuple[int, int]:
        """
        获得数字n位于素数列表的第几个位置，或者哪两个位置之间（1-based index！）
        :param n: 要搜索的素数
        :return: 如果搜到则返回index, index；搜不到则返回n的大小在哪两个相邻位置之间from, to
        """
        if n < 2:
            return 0, 1
        if n > self._list[-1]:  # 需要拓展列表
            self._extend(n)
        index = bisect.bisect_left(self._list, n)  # 二分法搜索n所处的位置
        if self._list[index] == n:
            return index + 1, index + 1  # 转换成1-based
        else:
            return index, index + 1  # 转换成1-based

    def primeRange(self, *args) -> islice:
        """
        获得素数数组的range
        三种调用方法：primeRange()，primeRange(stop)和primeRange(start, stop)。第一种方法等同于primeRange(INFINATY)
        :param args 传入的参数
        :return: 类似与range的迭代器
        """
        if len(args) == 0:
            ...
            return self.__getitem__(slice(args[0]))
        else:
            return self.__getitem__(slice(*args[:2]))

    def primeRange2(self, minN: int, maxN: int) -> Iterator:
        """
        返回包含[minN, maxN)素数的迭代器
        :param minN: 素数最小值
        ...
        i = self.indexOf(minN)[1]  # 找到startN所在的位置，或者startN可以插入的位置（1-based）
        maxi = len(self._list) + 1  # 最大i的取值（1-based）
        while i < maxi:  # 防止数组越界（其实不会，因为extend过了，一定是break之前就return）
            p = self._list[i - 1]  # 获取第i个元素（1-based）
